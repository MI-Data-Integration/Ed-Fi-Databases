...
{
    public class SqlServerCreateDatabaseCommand : IDatabaseCommand
    {
        ...
        public DatabaseCommandResult Execute(IOptions options)
        {
            ...
            _logger.Debug("Creating sqlserver database if it does not exist");

            EnsureDatabase.For.SqlDatabase(options.ConnectionString, new DbUpLogger(_logger));
